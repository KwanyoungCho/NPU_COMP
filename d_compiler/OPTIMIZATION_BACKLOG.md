...
- **남은 것 (파이썬 수준)**: `self.env`가 **TIR Var 객체로 키잉**되어 있어 조회마다
  FFI 해시가 난다(층당 약 380만 회, 남은 시간의 20~30% 추정). `handle.value`를
  키로 바꾸면 되지만 `env`를 쓰는 모든 지점을 손대야 한다
- **최종 목표**: **A6 (C++ codegen)**. 위 항목들은 전부 A6가 원천적으로 없앤다

### A6. codegen을 C++ TVM 백엔드로 — **최종 목표**
...
- **왜**: TVM의 실제 codegen(LLVM/CUDA/C)은 전부 `src/target/` 밑의 C++이고
  **FFI가 0회**다. 프로덕션 codegen을 파이썬으로 TIR 노드 단위로 걷는 사례는 없다.
  A5에서 확인했듯 우리 링크 시간의 대부분은 방출 자체가 아니라 FFI 왕복이며,
  이건 파이썬에 남아 있는 한 상수배로만 줄어든다
- **어떻게**: 커스텀 가속기용 공식 경로(BYOC / `target.build.<name>`)를 따른다.
  TVM은 이미 소스 빌드되어 있다(`~/tvm-src`)
- **기대치**: 손작성 backend 수준(수십만 word/s) → 전체 모델 링크 **분 단위**
- **전제**: 현재 파이썬 Walker가 **동작 정의(reference)** 역할을 한다. C++ 이식은
  같은 입력에 대해 **word 스트림이 bit-exact**한지로 검증한다
