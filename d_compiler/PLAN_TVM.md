...
| **S8** | **양자화를 Relax pass로** (현재 driver 실행 시 처리) | 🟡 (2026-08-31) **그래프 절반 완료**: `QuantizeWeightsW8A16`이 파라미터 weight의 matmul을 `scale → quantize → qmatmul`로 재작성한다. **`LiftTransformParams` 앞에** 두어 scale/packed weight를 표준 pass가 알아서 호스트로 hoist한다(토큰당 비용 0). llvm에서 float32 대비 **cosine 0.999998**(비양자화 0.999999), argmax 일치, lifted param 11→16(int8+float32). **남은 것 = NPU codegen 절반**: (a) qmatmul의 dequant epilogue를 `schedule_matmul_sram`이 못 받는다, (b) SRAM 주소가 원소당 4 nibble 고정이라 INT8(2)·FP32(8) 혼재를 못 다룬다, (c) 서술자 dtype·wscale 발행 미구현. W8A8은 그 다음 |
