...
**단, llvm 빌드는 우리보다 정밀도가 낮은 기준이다** (2026-08-28 확인).
TVM의 float16 matmul은 **float16으로 누적**하는데 우리 기계는 내부 누적이
FP32다. 실모델 차원 1층에서 float32 numpy 기준 채점 결과 NPU가
cosine 1.000000 / max|diff| 0.00008, llvm이 0.999965 / 0.00260이었고
마지막 행 argmax도 NPU만 기준과 일치했다. **둘이 어긋나면 float32 numpy로
판정한다** (`run_real_layer_npu.py`가 세 값을 함께 출력).
...
| **S6** | **target 등록 + `relax.build` 통합** | ✅ (2026-08-31) `npu_target()` = `ext_dev -keys=npu -model=v09` 실제 Target, 머신 상수는 `NpuProfile`(SRAM 8 MiB / tile 64 / lanes 256 / slot 6)로 모아 링커·스케줄이 읽는다. `relax.get_pipeline("npu")` 등록. `npu_target.build(mod, target)` → `NpuExecutable`(word 스트림 + 메모리 계획, `.run()`/`.save()`). **`relax.build`는 의도적으로 종점이 아니다** — 그 끝은 할당기와 호출 기제를 가진 런타임의 `runtime.Module`인데 이 기계엔 둘 다 없다(직선 스트림 + 단일 평면 이미지, 주소는 전부 컴파일타임 확정) |
| **S7** | **3-모델 end-to-end** (Gemma·Qwen3 nn.Module 추가) | 🟢 (2026-09-01) **Llama 28층**: token 358 + **HF logits cosine 0.9999927**(기존 golden 0.9999881 상회) — A1 적용 15,285,893 word(−51.0%), 실행 376초. **Qwen3 36층**: token 358 + **HF logits cosine 0.9999922** — 21,540,580 word(−49.8%), 실행 397초. **Gemma 35층**: llvm token 108 일치(frontend 확정), NPU는 binding-order 버그 수정 전 코드의 실행이 실패 → HEAD에서 2/5/16층 HF hidden 대비 0.999999/0.999999/0.999996, 전체 깊이 재실행 중 |
| **S8** | **양자화를 Relax pass로** (현재 driver 실행 시 처리) | ✅ (2026-08-31) `QuantizeWeightsW8A16`: 파라미터 weight matmul → `scale → quantize → dequant+matmul(단일 PrimFunc)`. `LiftTransformParams` **앞**에 두어 scale·packed weight가 호스트로 hoist(토큰당 비용 0), dequant+matmul은 한 커널이라 안 쪼개져 **global에는 INT8이 남는다**(DMA 트래픽 절반). 기계 매핑: INT8 타일 DMA → VDEQUANT(scale=1.0, fp32 상수) → FP16 scale row 벡터곱 → 검증된 FP16 gemm. C-model에서 **numpy mirror와 bit-exact**(64³·패딩 7×128×96 포함), tiny-Llama 전체 경로 float32 대비 cosine 0.999998. 이 과정에서 codegen이 dtype 혼재를 지원하게 됨(전역 주소 byte화, SRAM 폭 버퍼별) |
