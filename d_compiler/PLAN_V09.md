...
| **N6a** | W8A16: quantizer(pack) + INT8 weight 상주 실행 | ✅ (2026-08-26) 사다리 ①②③ 완료 — host reference와 bit-exact(whole·streaming 양 경로), 동일 문맥 logits cosine 0.9994+, 층별 진폭 변화 <5%; Gemma 3-token 전부 일치, Llama·Qwen은 각 1개 token이 **FP16 자체 1·2위 격차(0.031/0.047) < 양자화 RMSE(0.045/0.070)** 인 동률 지점에서 뒤집힘 |
| **N6b** | W8A8: VQUANT 시퀀스 + INT8 tile 곱 + dequant-in-matmul | ✅ (2026-08-26) host mirror와 bit-exact; 동일 문맥 logits cosine 0.986~0.998, **Gemma·Qwen3 3-token 전부 일치**, Llama 1개 token이 격차 0.047 < RMSE 0.187(0.25배) 동률 지점에서 뒤집힘 |
| **N7** | 통계·문서화: v09 vs 0818 비교표(word/DMA/SRAM) | ✅ (2026-08-26) 10-프로그램 세트 4개 타깃 측정 — words +26.9%(v09 FP16) / DMA 1,087.5→570.2 MiB(**−47.6%**, W8A16) / SRAM peak 792 KiB(용량의 10%). 양자화의 명령어 비용은 +0.3~0.9%p로 사실상 무시 가능. `report/report_v09.md` §5 |
