...
# Official Gemma 4 E2B text config (see report/SESSION_HANDOFF_0819.md §13).
GEMMA4_E2B = {
    "hidden_size": 1536,
...
def gemma4_e2b_spec(config=GEMMA4_E2B):
    """Spec for official Gemma 4 E2B text: 4 sliding + 1 full layer pattern,
    per-kind head dims/RoPE, and the last ``num_kv_shared_layers`` layers
    aliasing the latest owner slot of their own attention kind."""
    ...
    first_shared = num_layers - config["num_kv_shared_layers"]
    kinds = ["full" if index % 5 == 4 else "sliding" for index in range(num_layers)]
    last_owner = {}
    layers = []
