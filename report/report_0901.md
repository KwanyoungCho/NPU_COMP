...
**■ Memory Scheduling** — 무엇을 SRAM에 둘지 정하는 단계

- 제한된 SRAM(8 MiB)에 어떤 Tile/Intermediate를 적재·유지할지 결정
  - 가중치 텐서 하나가 18 MiB(`[3072,3072]` FP16)로 SRAM보다 큼
  - 따라서 "무엇을 상주시킬지"가 아니라 **"무엇을 타일 단위로 통과시키고,
    통과시킨 것 중 무엇을 남길지"**의 문제
  - v09 SRAM은 캐시가 아니라 **스크래치패드** → HW가 재사용을 대신해 주지
    않으므로 전부 컴파일러가 명시
- 데이터 재사용을 높이도록 연산 순서 및 SRAM Residency 결정
  - **연산 순서(루프 순서)가 곧 재사용 대상을 결정** — 최외곽 루프축에
    해당하는 피연산자가 안쪽 루프 내내 불변이므로 상주 후보가 됨
  - 상주 블록이 클수록 흘려보내는 쪽의 반복 횟수가 감소
    (`⌈M/Tm⌉`, `⌈N/Tn⌉`)
- SRAM 용량 초과 시 DRAM으로 Spill
  - HW 자동 spill 없음 → **명시적 DMA 명령으로 표현**
  - 특히 PE 누산기가 1개이므로, K 체인이 끊기면 부분합을 SRAM으로 spill 후
    재적재해야 하고 이때 **FP32 → FP16 정밀도 손실** 발생
  - → **K축 분할은 트래픽·정밀도 양쪽에서 손해이므로 회피 대상**

**■ Memory Allocation** — 정해진 것을 어디에 놓을지

- Buffer Lifetime을 기반으로 SRAM/DRAM 주소 할당
- Lifetime이 겹치지 않는 Buffer 간 메모리 영역 재사용
- v09 특성상 **런타임 할당기가 없어 모든 주소가 컴파일 타임 상수**
- 정렬 제약: 모든 할당은 32-bit 셀 경계로 올림(DMA가 셀 단위 이동),
  SRAM은 nibble 단위 · 8 nibble 정렬

#### (2) 기본적인 TVM의 memory planning

- **지원되는 것** — `StaticPlanBlockMemory`
  - `CallTIRRewrite`로 출력 버퍼가 명시화된 뒤, Buffer Lifetime 분석 수행
  - 생존구간이 겹치지 않는 텐서를 동일 storage pool에 배치 = **Storage 재사용**
  - 우리는 pool마다 flat 절대 주소만 부여 (실측: 활성 풀 1,599.7 MiB → 0.7 MiB)
  - → **Allocation 계층은 표준 pass를 그대로 재사용 가능**
- **지원되지 않는 것** — Scheduling 계층은 별도 설계 필요
  - **용량 제약 개념 없음** — 필요한 만큼 할당. DRAM 16 GiB에서는 충분했으나
    SRAM 8 MiB에서는 성립하지 않음
  - **선택 개념 없음** — 모든 텐서에 자리를 줌. "누구를 SRAM에 둘지" 고르지 않음
  - **트래픽 개념 없음** — 배치가 데이터 이동량에 미치는 영향을 모름
  - **커널 내부(Tile) 미대상** — 그래프 레벨 텐서만 다룸.
    커널 안의 타일 스테이징은 `tir.Schedule`의 영역

#### (3) 현재 구현의 문제점 — §15.4 실측 요약

- **(가) 원소별·정규화 커널이 텐서 전체를 스테이징** → seq 256에서 **링크 실패**
  (`mask_global.sram [24,256,256]` 9.00 MiB / 용량 8 MiB). 성능이 아니라
  **컴파일 가능 여부**의 문제
- **(나) matmul 활성화 피연산자가 열 타일마다 재적재** → seq 64에서 217.6 MB/층.
  역설적으로 패딩이 필요한 seq에서는 패딩 버퍼가 상주하여 1회로 끝남
- **(다) 가중치가 `⌈S/64⌉`회 재적재** → seq 128에서 2.01×
- **공통 원인**: SRAM 배치가 커널마다 0부터 bump이고, **8 MiB라는 한도를 보고
  무언가를 정하는 단계가 존재하지 않음**. 항상 동일하게 동작 —
  원소별은 "전부", matmul은 "타일 하나씩", 커널 종료 시 "전부 폐기"

#### (4) Memory Planning 구현 (설계)

**■ 설계 원칙**

- Allocation은 TVM 표준의 Lifetime·재사용 결과를 그대로 활용,
  **Scheduling만 신규 설계**
- 목적함수는 전량 **"이동 바이트"** → latency 모델 불필요,
  C-model 카운터(`dma_cells_loaded`)로 직접 검증 가능
- HW 제약을 먼저 고정하여 탐색 공간 축소
  - PE 타일 64×64 고정 → **타일 크기는 자유변수가 아님**
  - FP32 누산기 1개 → **K축이 최내곽**이어야 체인 유지
  - K축 미분할 원칙 → **3차원 탐색이 2지선다로 축소**

**■ 파이프라인 삽입 위치**

```
[2] 그래프 pass
[3] Global 메모리 계획   StaticPlanBlockMemory (Lifetime · pool)
[3.5] ★ SRAM 계획 ★     신설 — 상주 집합 선택 + 커널별 예산 산정
[4] 스케줄               [3.5]의 예산을 인자로 받아 blocking 결정
...
class SramPlan:
    ...
    resident: dict[str, int]      # 커널 간 상주 텐서 -> SRAM 주소
    budget:   dict[str, int]      # 커널 이름 -> 가용 nibble
```

**■ 계층 1: intra-op (커널 내부) — 커널 3종별 전략**

- **(가) 가중치 matmul** (q/k/v/o, gate/up/down, lm_head)
  - **최외곽 루프축 선택 = 상주 피연산자 선택**
    - 최외곽 = m(토큰) → A(활성화) 상주, B(가중치) 스트리밍
    - 최외곽 = n(출력채널) → B 상주, A 스트리밍
  - 트래픽 (`C[M,N] = A[M,K] @ B[K,N]`)

    | 선택 | A 트래픽 | B 트래픽 | 용량 |
    |---|---|---|---|
    | A 상주 | `M·K` (1회) | `K·N·⌈M/Tm⌉` | `2·Tm·K` |
    | B 상주 | `M·K·⌈N/Tn⌉` | `K·N` (1회) | `2·K·Tn` |

  - **선택 규칙: 작은 쪽을 상주**시키고 큰 쪽을 흘림.
    prefill은 `M(토큰) < N(출력채널)`이므로 대개 A 상주
  - 예산 8 MiB · K=3072 기준 `Tm ≈ 1300행` → **1300토큰까지 가중치 1회 적재**
  - 후보가 2개뿐이므로 **탐색이 아니라 두 식의 비교**로 결정
  - 예상 효과 (M=512, K=N=3072): **302 MB → 22 MB (13.7배)**, SRAM 3.5 MiB 사용

- **(나) 어텐션 matmul** (QK^T, probs·V)
  - 양 피연산자가 **모두 활성화** → "가중치냐 활성화냐"의 선택이 성립하지 않음
  - 중간 결과 `scores[H,S,S]`가 **S²로 증가**
  ...
  - **상주 선택으로 해결 불가** — 아무리 배치해도 들어가지 않음.
    현재 seq=256 링크 실패 지점이 정확히 이것
  - **해법: 전체 materialize 금지** — 행 블록 단위 계산 + 온라인 softmax 갱신
    (flash attention과 등가 변환)
  - 성격: 배치 문제가 아니라 **연산 순서 재구성** → 스케줄·그래프 재작성 영역

- **(다) 원소별·정규화** (rms_norm, silu, add, softmax)
  - 스트리밍 구조로 **상주 대상 없음**
  - 현재 텐서 전체 스테이징 → seq 512에서 `rms_norm [512,3072]` 9.04 MiB로 실패
  - **해법: 행 축 `split` + `compute_at`** 으로 "예산에 들어가는 행 수"만 스테이징

**■ 계층 2: inter-op (커널 간 Residency)**

- **대상**: 커널이 생산하고 후속 커널이 소비하는 활성화
  - **residual이 대표 사례** — 생존구간이 층 전체에 걸치고 소비자가 2회 이상
- **이득·비용**
  - 이득 = `크기 × (쓰기 1회 + 소비 커널 수)` 만큼의 DMA 제거
  - 비용 = 생존구간 동안 SRAM 점유
- **선택 알고리즘**
  - 밀도(바이트당 절감) 기준 그리디
  - 제약: **생존구간에 속한 모든 커널**에서
    `상주 점유 + 해당 커널 intra-op 예산 ≤ 8 MiB`
  - 구간 그래프 위 배낭 문제이나, 커널이 사슬 구조이고 생존구간이 짧아 그리디로 충분
- **구현**
  - 링커가 이미 모든 SRAM 주소를 소유 → **인자 바인딩만 전환**

    ```python
    if name in sram_plan.resident:
        walker.declare_sram(buffer, sram_plan.resident[name])   # SRAM 직접 참조
    else:
        walker.bases[buffer.data] = address                     # 기존 DRAM 경유
    ```

  - 스케줄에는 상주 사실을 통보하여 해당 피연산자의 `cache_read`를 생략
  - → **TVM이 커널 간 상주를 표현해 줄 필요 없음**

**■ 예산 중재 (intra-op ↔ inter-op)**

- 양 계층이 동일한 8 MiB를 사용하므로 배분 필요
- intra-op 이득은 `⌈⌉`로 인해 **계단형** — 다음 계단에 닿기 전에는 추가 배정
  효과 없음
- inter-op 이득은 텐서 단위로 **일정**
- → **한계이득(SRAM 1 byte당 절감 바이트) 기준 그리디가 최적**
- 실무적으로는: **intra-op에 다음 계단까지만 배정하고 잔여를 inter-op에 배정**
  - M=512 기준 intra-op은 3.5 MiB로 이미 최선 → **4.5 MiB가 inter-op으로**

**■ Spill 정책**

- 상주 선정에서 탈락한 텐서는 기존과 동일하게 DRAM 왕복 (기능 저하 없음)
- 커널 내 부분합 spill은 **K축 미분할 원칙으로 원천 회피**

#### (5) 구현 순서 및 검증

| 순서 | 작업 | 해결 항목 | 기대 효과 | 검증 |
|---|---|---|---|---|
| 1 | 원소별·정규화 행 블록화 | (다) | **긴 프롬프트 컴파일 가능** | seq 512 링크 성공 |
| 2 | 어텐션 행 블록 + 온라인 softmax | (나) | 긴 컨텍스트 가능 | seq 512 링크 + 수치 대조 |
| 3 | 가중치 matmul 상주 blocking | (가) | **13.7배** | 트래픽 감소 |
| 4 | inter-op 상주 + 예산 중재 | — | 왕복 제거 | `gstore` 감소 |
| 5 | (보류) double buffering | — | 전송·연산 중첩 | ISA에 async DMA 필요 |

- **검증 도구 (기구축)**
  - `d_compiler/analyze_traffic.py` — 실행 없이 링크만으로 트래픽·SRAM peak 산출.
    28층에서 C-model 카운터와 **1,619,976,421 cell로 일치 확인**
  - `tests/test_native_codegen.py` — codegen 무결성 (word 단위 대조)
  - 전체 깊이 게이트 4종 — 수치 정확성
- **추가 필요**
  - **긴 프롬프트 수치 정확성 테스트** — 현재 게이트가 전부 seq=7이므로
    1~4의 개선도 회귀도 검출되지 않음. seq 128~512 llvm 대조를 1번과 함께 구축
