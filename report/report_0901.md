...
### 15.5 메모리 Planning 설계

#### (1) 메모리 planning 단계에서 해결해야 하는 부분

**■ Memory Scheduling** — 무엇을 SRAM에 둘지 결정

- 제한된 SRAM(8 MiB)에 어떤 Tile/Intermediate를 적재·유지할지 결정
  - 가중치 텐서 하나가 18 MiB(`[3072,3072]` FP16)로 SRAM보다 큼 → "무엇을
    상주시킬지"가 아니라 **"무엇을 어느 단위로 통과시키고 무엇을 남길지"**의 문제
  - v09 SRAM은 캐시가 아니라 **스크래치패드** → HW가 재사용을 대신하지 않으므로
    전량 컴파일러가 명시
- 데이터 재사용을 높이도록 연산 순서 및 SRAM Residency 결정
  - **연산 순서(루프 순서)가 재사용 대상을 결정** — 바깥 루프축에 불변인
    피연산자가 상주 후보
- SRAM 용량 초과 시 DRAM으로 Spill
  - HW 자동 spill 없음 → **명시적 DMA로 표현**

**■ Memory Allocation** — 정해진 것을 어디에 놓을지

- Buffer Lifetime 기반 SRAM/DRAM 주소 할당
- Lifetime이 겹치지 않는 Buffer 간 영역 재사용
- 런타임 할당기가 없어 **모든 주소가 컴파일 타임 상수**
- 정렬 제약: 32-bit 셀 경계 올림(DMA 단위), SRAM은 nibble 단위·8 nibble 정렬

#### (2) 기본적인 TVM의 memory planning

- **지원** — `StaticPlanBlockMemory`
  - Buffer Lifetime 분석 및 Storage 재사용 (실측: 활성 풀 1,599.7 MiB → 0.7 MiB)
  - → **Allocation 계층은 표준 pass를 그대로 재사용**
- **미지원** — Scheduling 계층은 별도 설계 필요
  - **용량 제약 없음** · **선택 개념 없음** · **트래픽 개념 없음** ·
    **커널 내부(Tile) 미대상**
  - DRAM 16 GiB에서는 불필요했던 것들이며, SRAM 8 MiB에서는 전부 필수

#### (3) 설계의 출발점 — 단일 구조로 정의

- **문제의 관찰**: §15.4에서 드러난 세 결함(원소별 커널의 전량 스테이징,
  matmul 피연산자 재적재, 커널 간 왕복)은 **서로 다른 문제가 아님**
  - 모두 **"이 피연산자를 어느 루프 깊이에서 SRAM에 올릴 것인가"** 하나로 환원
  - 현재는 그 선택이 전부 **고정값**으로 박혀 있을 뿐
- **따라서 단일 결정변수와 단일 비용모델로 정의하고, 커널 종류별 차이는
  설계가 아니라 그 모델의 해(解)로 나오게 한다**

**■ 단일 결정변수 — Residency Level**

- 커널의 루프 중첩을 바깥부터 `L0, L1, …, Ln`(최내곽)이라 할 때, 각
  **(커널, 피연산자)** 쌍에 하나의 정수를 배정

```
level(kernel, operand) ∈ { −1, 0, 1, …, n }

  level = −1  : 커널 밖 상주 (inter-op residency) — 스테이징 자체가 없음
  level =  j  : 루프 Lj 진입 시점에 스테이징
  level =  n  : 최내곽 스테이징 = 현재 구현의 동작
```

- 루프 분할(blocking factor)은 **레벨을 새로 만드는 행위**이므로 같은 변수에 포함

**■ 단일 비용모델**

```
footprint(X, j) = 레벨 j 이하 루프에 대해 불변인 X 슬라이스의 크기
traffic  (X, j) = footprint(X, j) × (레벨 L0…Lj−1 반복 횟수의 곱)

          단,  level = −1 이면  footprint = 전체 크기,  traffic = 0
```

- 식이 하나이고, 세 결함이 모두 이 식의 특수해로 표현됨

| 사례 | level | footprint | traffic |
|---|---|---|---|
| matmul A를 행블록 루프에 올림 | `m_b` | `Tm·K` | `M·K` (1회) |
| matmul B를 최내곽에 올림 (현재) | `k_o` | `64·64` | `K·N·⌈M/Tm⌉` |
| 원소별 입력을 행블록 루프에 올림 | `r_o` | `rows·W` | `M·W` (1회) |
| 원소별 입력을 루프 밖에 올림 (현재) | `L0` 이전 | `M·W` | `M·W` |
| residual을 커널 밖 상주 | `−1` | 텐서 전체 | `0` |

**■ 단일 최적화 문제**

```
minimize   Σ over (커널 k, 피연산자 X)   traffic(X, level(k,X))

s.t.  모든 프로그램 지점 p 에 대하여
        Σ_{p에서 살아있는 상주 텐서} 크기
      + Σ_{p의 커널 피연산자}        footprint(level)
      + 고정 영역(상수풀 · scratch)
      ≤ 8 MiB
```

- 목적함수가 **이동 바이트 하나** → **latency 모델 불필요**.
  검증도 C-model 카운터(`dma_cells_loaded`)로 직접 수행
- 제약이 **프로그램 지점별**이므로 intra-op(커널 내부)과 inter-op(커널 간)이
  자동으로 같은 예산을 두고 경쟁 → **별도의 예산 중재 규칙이 필요 없음**

**■ 탐색 공간을 줄이는 HW 제약 (먼저 고정)**

- PE 타일 64×64 고정 → **타일 크기는 자유변수가 아님**
- **FP32 누산기 1개** → K축이 최내곽이어야 MAC 체인 유지
- K축 분할 시 부분합을 SRAM으로 spill 후 재적재하며 **FP32 → FP16 손실** 발생
  → **K축 미분할을 원칙으로 고정**
- 결과: 피연산자당 후보 레벨이 **2~3개**로 축소 → **탐색이 아니라 완전 열거**

#### (4) Memory Planning 구조

```
        ┌──────────────────────────────────────────────────────┐
        │ [3] Global 계획   StaticPlanBlockMemory               │
        │     Buffer Lifetime · Storage 재사용                  │
        └───────────────────────┬──────────────────────────────┘
                                │ lifetime, pool, 텐서 크기
                                ▼
   ┌────────────────────────────────────────────────────────────────┐
   │ [3.5] SRAM Planner                                    (신설)    │
   │                                                                │
   │   ① 정규화   커널 → (반복공간, 피연산자, 축 의존성)              │
   │   ② 비용모델  level → (footprint, traffic)     ← 단일 식         │
   │   ③ 배정     한계이득 기준으로 level·blocking·상주를 동시 결정    │
   │                                              → SramPlan        │
   └───────────────┬────────────────────────────────────▲───────────┘
                   │ budget · level · resident          │
                   ▼                    (Loop B) 실제 footprint 회신
   ┌────────────────────────────────────────────────────┴───────────┐
   │ [4] 스케줄   split / reorder / cache_read / compute_at          │
   │              + CompactBufferAllocation                          │
   └───────────────┬────────────────────────────────────▲───────────┘
                   │                     (Loop C) 최소 blocking에서도
                   │                              infeasible → 그래프 재작성
                   ▼
   ┌────────────────────────────────────────────────────────────────┐
   │ [5] codegen / 링크    상주 텐서는 SRAM 주소로 바인딩              │
   └───────────────┬────────────────────────────────────────────────┘
                   │
                   ▼  (Loop D) analyze_traffic · C-model 카운터 → 모델 검증
```
...
@dataclass
class SramPlan:
    capacity: int                            # nibbles (8 MiB)
    level:    dict[(kernel, operand), int]   # residency level (−1 = 커널 밖 상주)
    blocking: dict[(kernel, axis), int]      # 축별 분할 인수
    resident: dict[str, int]                 # 상주 텐서 -> SRAM 주소
    budget:   dict[str, int]                 # 커널 -> 가용 nibble (스케줄 인자)
```

**■ 배정 알고리즘 (③)**

- 초기 배정: 모든 피연산자를 최내곽(`level = n`)에 두면 footprint 최소 · traffic 최대
- 이후 **한계이득이 큰 순서로 레벨을 한 칸씩 바깥으로 이동**
  - 한계이득 = `Δtraffic / Δfootprint` (SRAM 1 byte당 절감되는 이동 바이트)
  - `level = −1`(커널 밖 상주)도 동일 지표로 경쟁 → intra/inter가 한 큐에서 정렬됨
  - 제약(프로그램 지점별 용량)을 위반하는 이동은 건너뜀
- 목적함수가 분리 가능하고 각 항이 레벨에 대해 오목(계단 감소)하므로
  **한계이득 그리디가 최적**
- 후보 수가 작아 커널 단위는 완전 열거로도 가능

**■ 모델이 내놓는 해 (설계가 아니라 결과)**

| 커널 형태 | 배정 결과 | 근거 |
|---|---|---|
| 가중치 matmul | 작은 피연산자를 `L0`에, 큰 쪽을 최내곽에 (prefill은 `M<N`이므로 대개 활성화가 상주) | 작은 쪽을 올려야 footprint당 절감이 큼 |
| 원소별·정규화 | 입력·출력을 행블록 루프 레벨에 (`rows = 예산 / 행 폭`) | 상주해도 traffic이 안 줄어들므로 footprint 최소로 수렴 |
| 커널 간 활성화·residual | `level = −1`, 소비자 수가 많고 생존구간이 짧은 것부터 | 한계이득 = `크기×(1+소비자수) / 크기` = 소비자 수 |

- 즉 앞서 사례별로 논의하던 전략들이 **같은 지표의 정렬 결과**로 나옴

#### (5) 피드백 루프

- 이 planner는 **개방 루프로 완결되지 않음** — 세 지점에서 회신이 필요

**■ Loop B — 실현 footprint 보정 (컴파일 내부, 1~2회)**

- **필요 이유**: planner의 footprint는 해석적 추정이나, 실제 버퍼는 스케줄 이후
  `pad_einsum`의 패딩 버퍼, `CompactBufferAllocation`의 축소 결과,
  커널별 scratch slot 폭(`_scratch_row`)이 확정되어야 정해짐 → **추정과 실측이 다름**
- **동작**: 스케줄 후 `_place_sram`으로 실제 점유를 측정하여 planner에 회신
  - 초과 → 해당 커널의 blocking을 한 단계 낮춰 재스케줄
  - 여유가 큼 → 한 단계 올려 재스케줄
- **비용**: 스케줄은 **커널 종류당 1회만 수행되고 캐시됨**(28층 실측 6.3s)이므로
  재시도가 저렴
- **현재와의 차이**: 지금은 초과 시 `LinkError`로 **중단**. 이를 **재계획 신호**로
  전환하는 것이 이 루프의 실체

**■ Loop C — 문제 재정의 (드묾)**

- **필요 이유**: 최소 blocking에서도 제약을 만족하지 못하는 경우가 존재
  - 실제 사례: 어텐션의 `scores[H,S,S]`가 S²로 증가 (S=512에서 12.6 MB > 8 MiB).
    **어떤 level 배정으로도 해결 불가**
- **동작**: planner가 *infeasible*을 보고 → **그래프 재작성**을 호출
  - QK^T · softmax · PV를 하나로 묶고 행 블록 단위로 계산 + 온라인 softmax 갱신
    (flash attention과 등가)
  - 재작성된 그래프로 [3]부터 재수행
- **의의**: 어텐션을 예외 케이스로 두지 않고, **"spill로 못 푸는 경우의 표준 응답"**
  으로 구조에 편입

**■ Loop D — 비용모델 검증 (개발 시 회귀 게이트)**

- **필요 이유**: 비용모델이 실제 이동량과 어긋나면 배정 전체가 무의미
- **동작**: 예측 traffic ↔ 실측 비교
  - `analyze_traffic.py` — 링크된 프로그램의 DMA 명령 합산 (실행 불필요)
  - C-model 카운터 `dma_cells_loaded` — 28층에서 **1,619,976,421 cell로 일치 확인됨**
- 컴파일 타임 루프가 아니라 **CI 게이트**로 운용

**■ 불필요한 루프**

- **예산 중재 루프는 불필요** — intra-op과 inter-op을 별도로 풀고 반복 조정하는
  구조를 검토했으나, 제약을 **프로그램 지점별**로 세우면 한계이득 그리디 한 번에
  동시 배정되므로 반복이 발생하지 않음
- **탐색 루프(auto-tuning)도 현 단계에서는 불필요** — HW 제약으로 후보가 2~3개로
  줄어 완전 열거가 가능. MetaSchedule은 Loop B·C가 안정된 뒤 검토 대상

#### (6) 구현 순서 및 검증

| 순서 | 작업 | 구조상 위치 | 기대 효과 | 검증 |
|---|---|---|---|---|
| 1 | 원소별·정규화의 행 레벨 배정 | ③ 배정 + [4] 스케줄 | **긴 프롬프트 컴파일 가능** | seq 512 링크 성공 |
| 2 | Loop C (어텐션 그래프 재작성) | 피드백 | 긴 컨텍스트 가능 | seq 512 링크 + 수치 대조 |
| 3 | matmul 레벨 배정 | ②③ | **13.7배** (302 MB → 22 MB) | 트래픽 감소 |
| 4 | `level = −1` (커널 간 상주) 도입 | ③ + [5] 링크 | 왕복 제거 | `gstore` 감소 |
| 5 | Loop B (실현 footprint 보정) | 피드백 | 추정 오차 흡수 | SRAM peak 수렴 |
| 6 | (보류) double buffering | — | 전송·연산 중첩 | ISA에 async DMA 필요 |

- **기구축 검증 도구**
  - `d_compiler/analyze_traffic.py` — 실행 없이 트래픽·SRAM peak 산출, C-model 일치 확인
  - `tests/test_native_codegen.py` — codegen 무결성 (word 단위 대조)
  - 전체 깊이 게이트 4종 — 수치 정확성
- **추가 필요**
  - **긴 프롬프트 수치 정확성 테스트** — 현재 게이트가 전부 seq=7이므로 1~5의
    개선도 회귀도 검출되지 않음. seq 128~512 llvm 대조를 1번과 함께 구축
