...
| 2026-08-03 | **실 3B overhead 측정 — A4-5d 갭 발견** | 3-R | ❗ 실 Llama 3B(HD=128) 컴파일 후 role 측정: v2 tile이 gather를 크게 줄이나(16.8M→3.5M) v1 "after"(gather=0, 1.06M)엔 못 미침 — **residual/RMSNorm/RoPE가 row로 남아** QKV 40·FFN 2 projection이 activation(xn/hn)을 gather(2.16M 잔존). 앞선 "minor 보류" 오판 정정 → Stage 4 착수 |
| 2026-08-03 | **Stage 4 feasibility spike (workflow, 5 agents)** | layout_transform-native A4 가능? | ✅ **GO**. packed matmul `[Mt,Kt,64,64]@[Kt,Nt,64,64]`를 기존 `npu_gemm_acc`로 tensorize(LLVM+walker exact, `_bind_match` ~2줄). RoPE/transpose/slice/concat도 layout_transform로는 tile 가능(row-only는 수제 emitter 탓). **핵심 판단: gather 닫는 데 전체 재아키텍처 불필요 — Phase 4.1(fixpoint 완화)이 최저 위험으로 대부분 닫음** | workflow w5crt9bx4 |
| 2026-08-03 | **Stage 4 Phase 4.0/4.2a — packed matmul in production** | 3-R | ✅ `packed_matmul`(einsum nest → `npu_gemm_acc` tensorize) + `emit_packed_matmul` + `_bind_match` 4D leading-stride 수정(현 경로 무영향). fully-tile matmul을 실 경로에서 packed nest로(byte-exact vs `emit_matmul_into`). `test_v2_packed_matmul` | v2_backend.py |
| 2026-08-03 | **Stage 4 Phase 4.3 — F4 pre-legalize packed 패스 (workflow 2 agents)** | 3-R | ✅ `npu_compiler/packed.py:_build_packed` — high-level Relax를 packed 4D로 rewrite(matmul→einsum, ew/reduce/broadcast on [Rt,Ct,64,64], RoPE=row island via in-graph layout_transform). **relax-VM 검증: packed vs row graph bit-identical(0.0), vs ref_layer<0.05, 6 config, CONFIRMED_EQUIVALENT.** gotcha: size-1축 layout_transform 패딩 버그 → vector는 reshape. **정직한 갭: LegalizeOps가 einsum/te_layout_transform/reshape/4D-ops를 내는데 현 `_emit`은 이를 lower 못 함 → NPU-ISA 통합(packed-aware walker)이 Phase 4.4+의 실질 본체** | packed.py, test_v2.py |
| 2026-08-03 | **Stage 4 Phase 4.1 — residual 스트림 tile** | 3-R | ✅ **한 줄 근본수정**: fixpoint 664행 `demote=(v==out_name)`이 64-mult 출력을 무조건 row로 끌어 residual 클러스터 전체를 collapse시킴 → `demote=(v==out_name and not _is64_2d)`로 64-mult 출력은 tile 유지(host unpack). residual/RMSNorm/FFN이 tile end-to-end(x·out=tile) → **실 3B gather 2.16M→98K(−95%)**, total 3.5M→1.29M(v1 after 1.06M 근접). 정확성 rel 0.0047~0.028 vs ref_layer. gate GREEN, `test_v2_residual_tile_gather` 편입 | v2_backend.py:664, test_v2.py |
