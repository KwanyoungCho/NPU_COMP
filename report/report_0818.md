...
- **(2026-08-21 추가 실증) 제공 b_program 예제 스위트가 이 함정에 걸려 있음**:
  vector 예제 46개의 주소 설정 298개가 **전부 MAIN**(bit28=0)으로 되어 있어,
  실행기에서는 주소 지정이 무시되고 stale PARTIAL(초기 0)에서 동작한다.
  vendor `a.out` 직접 실행으로 확인: `inst_0001_vector_add_immediate`는 의도
  (주소 5 → +3 → 주소 32)와 달리 **주소 0에서 읽어 주소 0에 저장**했다.
  matrix 예제도 `inst_1003` 하나를 제외하면 PARTIAL 미설정(대부분
  partial_rows=0으로 사실상 no-op). 이것이 V3-007의 실체다. 기존 parity 통과는
  "두 실행기가 동일하게 동작함"의 증명이지 "예제가 의도대로 동작함"의 증명이
  아니다. **설계팀 확인 필요**: 실행기(PARTIAL 소비)가 스펙 의도라면 예제
  스위트 재작성+PDF 명문화, 예제가 의도라면 실행기의 bit28 해석이 버그.
