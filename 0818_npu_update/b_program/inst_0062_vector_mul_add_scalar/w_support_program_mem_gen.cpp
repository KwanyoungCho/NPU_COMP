...
int main(int argc, char **argv)
{
    ...
    std::string instruction_32bit_array_string = ""; 
     ...
     instruction_vec[5] = (0x1 << 30) + (5 << 8) + 0x01;    // vector add, scalar
     instruction_vec[6] = (0x1 << 30) + (5 << 8) + 0x0C;    // vector multiply-add, scalar
     instruction_vec[7] = (2 << 30) + (0 << 29) + (32 << 8) + 0x80;    // Global buffer start address, 1st, low
     instruction_vec[8] = (2 << 30) + (1 << 29) + (0 << 8) + 0x80;    // Global buffer start address, 1st, high 
     instruction_vec[9] = (0 << 31) + 0x98;    // save, vector
     instruction_vec[10] = (0 << 30) + (0 << 29) + (32 << 8) + 0x80;    // Global buffer start address, 1st, low
     instruction_vec[11] = (0 << 30) + (1 << 29) + (0 << 8) + 0x80;    // Global buffer start address, 1st, high 
     instruction_vec[12] = (8 << 8) + 0x82;    // Global buffer length, Vector
     instruction_vec[13] = (0 << 31) + (0 << 30) + 0x90;    // load, 1st, vector
