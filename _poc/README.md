...
> 기본 분석 기준에서는 vendor의 관찰된 quirk를 그대로 유지하고 program file 전체를
> 동적으로 읽는다. compiler-owned full-model 실행에서는 같은 flat-address 의미를 유지한
> 채 G-buffer만 8192 entry 밖으로 안전하게 확장한다.
